...
import cn.mahjong.utils.tag.easyui.vo.Tab;
...
public class TabsTag extends TagSupport {
	...
	public StringBuffer end() {
		...
		if (iframe) {
			...
			sb.append("{");
			sb.append("var s = \'<iframe id=\"\'+id+\'\" scrolling=\"no\" frameborder=\"0\"  src=\"about:jeecg\" width=\""+ StringUtils.defaultIfBlank(width, "100%")+"\" height=\""+StringUtils.defaultIfBlank(heigth, "99.5%")+"\"></iframe>\';");
			sb.append("return s;");
			sb.append("}");
			sb.append("});");
			sb.append("</script>");
		...
		if (tabs) {
			...
			sb.append("<div id=\"" + id + "\" tabPosition=\"" + tabPosition + "\" border=flase style=\"margin:0px;padding:0px;overflow:hidden;width:"+StringUtils.defaultIfBlank(width, "auto")+";\" class=\"easyui-tabs\" fit=\""+fit+"\">");
			if (!iframe) {
				for (Tab tab : tabList) {
					if (tab.getHref() != null) {
						...
						sb.append("<iframe id=\"\'"+tab.getId()+"\'\" scrolling=\"no\" frameborder=\"0\"  src=\""+tab.getIframe()+"\" width=\""+StringUtils.defaultIfBlank(tab.getWidth(), "100%")+"\" height=\""+StringUtils.defaultIfBlank(tab.getHeigth(), "99.5%")+"\"></iframe>\';");
						sb.append("</div>");
					}
