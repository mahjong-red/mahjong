...
import cn.mahjong.utils.tag.easyui.vo.ColumnValue;
import cn.mahjong.utils.tag.easyui.vo.DataGridColumn;
import cn.mahjong.utils.tag.easyui.vo.DataGridUrl;
import cn.mahjong.utils.tag.easyui.vo.OptTypeDirection;
...
public class DataGridTag extends TagSupport {
	 ...
	 * @param value
	 */
	public void setColumn(String title, String field, Integer width, String rowspan, String colspan, String align, boolean sortable, boolean checkbox, String formatter, boolean hidden,
			String replace, String treefield, boolean image, String imageSize, boolean query, String url, String funname, String arg, String queryMode, String dictionary, boolean frozenColumn,
			String extend, String style, String downloadName, boolean isAuto, String extendParams) {
		...
		if (!StringUtils.isBlank(dictionary)) {
			...
			// String text = "";
			// String value = "";
			// List<TSType> typeList =
			// TSTypegroup.allTypes.get(dictionary.toLowerCase());
	...
	public StringBuffer datatables() {
		...
		sb.append("\"oLanguage\" : {" + "\"sLengthMenu\" : \" _MENU_ 条记录\"," + "\"sZeroRecords\" : \"没有检索到数据\"," + "\"sInfo\" : \"第 _START_ 至 _END_ 条数据 共 _TOTAL_ 条\"," + "\"sInfoEmtpy\" : \"没有数据\","
				+ "\"sProcessing\" : \"正在加载数据...\"," + "\"sSearch\" : \"搜索\"," + "\"oPaginate\" : {" + "\"sFirst\" : \"首页\"," + "\"sPrevious\" : \"前页\", " + "\"sNext\" : \"后页\","
				+ "\"sLast\" : \"尾页\"" + "}" + "},"); // 汉化
...
//		sb.append("});");
	...
	private String getAutoSpan(String filed, String extend) {
		...
		nsb.append("$(document).ready(function() {").append("$(\"#" + getEntityName() + "_" + id + "\").autocomplete(\"commonController.do?getAutoList\",{")
		...
		nsb.append("$(\"#" + getEntityName() + "_" + id + "\").val(row['" + filed + "']);}); });").append("</script>")
